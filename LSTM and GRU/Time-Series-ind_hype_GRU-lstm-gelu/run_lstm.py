...
def main():
   ...
   #训练结果
    x_train, y_train = data.get_train_data(#修改+
        seq_len=configs['data']['sequence_length'],
        normalise=configs['data']['normalise']
    )
    # 展示拟合结果,修改+
    ...
    # 所有结果,修改+
    x_full, y_full = data.get_full_data(
        seq_len=configs['data']['sequence_length'],
        normalise=configs['data']['normalise']
    )
    # 展示full结果,修改+
    predictions_pointbypoint = model.predict_point_by_point(x_full, debug=True)
    # print(len(predictions_pointbypoint))
    # print(len(y_train))
    # print(y_train[:, 0])
    MSE = metrics.mean_squared_error(y_full[:, 0], predictions_pointbypoint)
    MAE = metrics.mean_absolute_error(y_full[:, 0], predictions_pointbypoint)
    print('MSE_full', MSE)
    print('MAE_full', MAE)
    # plot_results_multiple(predictions_multiseq, y_test, configs['data']['sequence_length'])
    plot_results_test(predictions_pointbypoint, y_full)
